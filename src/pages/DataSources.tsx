...
import { dataSourceCouncilUsage } from "@/lib/mockCouncilData";
import { formatDistanceToNow } from "date-fns";
import { Activity, AlertCircle, CheckCircle2, Scale } from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4">
        {mockDataSources.map((source) => {
          const councilUsage = dataSourceCouncilUsage[source.id];
          return (
            <Card key={source.id}>
              <CardHeader>
                        ...
                        <Tooltip>
                          ...
                          <TooltipContent side="top" className="max-w-xs">
                            <div className="space-y-1">
                              <p className="font-medium">Used by {councilUsage.usedByModels.length} models</p>
                              <p className="text-xs text-muted-foreground">
                                {councilUsage.usedByModels.join(', ')}
                              </p>
                              <p className="text-xs">
                                Importance: <span className="font-medium">{councilUsage.importance}</span>
                              </p>
                            </div>
                          </TooltipContent>
                        </Tooltip>
